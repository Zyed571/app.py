...
if st.session_state.current_page == 2:
    ...
    # Display referred doctors - Each on a new line, without numbering
    if st.session_state.referred_doctors:
        st.write("**Referred By**:")
        for doctor in st.session_state.referred_doctors:
            st.write(doctor)  # Each doctor on a new line

    st.markdown("### Selected Tests and Amounts:")
    total_amount = 0
    for test, price in st.session_state.selected_tests:
        st.write(f"{test} - ₹{price}")
        total_amount += price
    ...
    st.markdown("_____________", unsafe_allow_html=True)  # Signature line
